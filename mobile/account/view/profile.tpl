<script>
	var len=$("#js-applist .app").length;
	if (len>=5){
		$("#js-applist .app").each(function(){
			if ($(this).index()>=5){
				$(this).hide();
			}
		});

		$("<div class='more'>Xem thêm</div>").appendTo("#js-applist").on("click", function(){
			$("#js-applist .app").show();
			$(this).hide();	
		});
	}
...
<div id='profile'>
	...
	<div class='list'>
		...
		<div class='applist' id='js-applist'>
			
			<?php 
				$apps=\base\AppList::get(\Client::$viewer);
				foreach ($apps as $app){
					$url=$app->path.".".DOMAIN;
					if (!$app->path){
						$url=DOMAIN;
					...
					echo "<a class='app' target='_blank' href='https://{$url}'>
					<span class='image'><img src='".SHARE_URL."/apps/{$app->icon}'></span>
					<span class='text ap-xdot'>{$app->name}<em class='ap-xdot'>{$app->title}</em></span>
					</a>";
				}
			?>
		</div>
